...
const AxisView = (props) => { // axesTheta, setAxesTheta, setCheckViz, attr
	...
	useEffect(() => {
		...
		const groups = svg.selectAll(".dragGroup").data(props.attr.map((d, i) => {return {attr: d, theta: props.axesTheta[i], index: i}}))
			.enter()
			.append("g")
		...
		groups
			.append("path")
				.attr("d", (d) => {
					...
					path.lineTo(center + bigRadius * Math.cos(d.theta), center - bigRadius * Math.sin(d.theta));
					return path;
				})
